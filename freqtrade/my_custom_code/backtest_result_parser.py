...
def extract_data_from_file(file_content, filename):
    ...
    while i < len(lines):
            ...
            for metric, value in metrics.items():
                ...
                # Add Chinese explanation to metric
                    if metric == 'Backtesting from':
                        ...
                        metric_display = f"Trading_Mode_(交易模式)"  # 交易模式：回测使用的交易模式（如现货、期货） Spot 现货 ｜ Futures 期货
                    elif metric == 'Max open trades':
                        metric_display = f"Max_open_trades_(最大同时持仓交易数)"  # 最大同时持仓交易数：回测中允许的最大同时开放交易数量
                    elif metric == 'Total Daily Avg Trades' or metric == 'Total/Daily Avg Trades': # Total/Daily Avg Trades
                        metric_display = f"Total_Daily_Avg_Trades_(日均交易次数)"  # 日均交易次数：每日平均交易数量 133 / 0.07  总交易次数 /  比如交易时间一共有1118天  就是133/1118每天交易次数
                    elif metric == 'Starting balance':
                        metric_display = f"Starting_balance_(初始余额USDT)"  # 初始余额：回测开始时的账户余额 1000 USDT
                    elif metric == 'Final balance':
                        metric_display = f"Final_balance_(最终余额USDT)"  # 最终余额：回测结束时的账户余额 666.669 USDT
                    elif metric == 'Absolute profit':
                        metric_display = f"Absolute_profit_(总利润USDT)"  # 绝对利润：回测期间的总绝对盈利
                    elif metric == 'Total profit %':
                        metric_display = f"Total_profit_Pct_(总利润百分比)"  # 总利润百分比：整个回测的总盈利百分比
                    elif metric == 'CAGR %':
                        metric_display = f"CAGR_Pct_(年化收益率)"  # 年化收益率：复合年增长率 (Final/Start)^(1/Y)−1  
                    elif metric == 'Sortino': # 平均收益率 / 下行波动率
                        metric_display = f"Sortino_(索提诺比率)"  # 索提诺比率：衡量下行风险调整后的回报 越大越好，承受1单位的下行波动，能得到多少正向收益。夏普是考虑上涨+下跌，
                    elif metric == 'Sharpe':  # 均值/方差，越大越好
                        metric_display = f"Sharpe_(夏普比率)"  # 夏普比率：衡量风险调整后的回报 
                    elif metric == 'Calmar': # 年化/最大回撤 ，每承受1单位回撤风险，得到多少年化回报
                        metric_display = f"Calmar_(卡尔马比率)"  # 卡尔马比率：衡量最大回撤调整后的回报， 越大高回报小回撤
                    elif metric == 'SQN':
                        metric_display = f"SQN_(系统质量数)"  # 系统质量数：衡量交易系统质量的指标  大于1.6左右。 赚的稳定不稳定，能不能活下来
                    elif metric == 'Profit factor':
                        metric_display = f"Profit_factor_(盈利因子)"  # 盈利因子：总盈利与总亏损的比率 盈利金额/亏损金额， 每亏损1元帮你赚多少钱
                    elif metric == 'Expectancy Ratio' or metric == 'Expectancy (Ratio)': # Expectancy (Ratio)
                        metric_display = f"Expectancy_Ratio_(期望比率)"  # 期望比率：每笔交易的平均预期回报  每做一笔交易，我平均能赚（或亏）多少风险单位的收益？
                        amount, ratio = re.findall(r"[-+]?\d*\.\d+|\d+", value)
                        amount = float(amount)
                        ratio = float(ratio)
                        data[f"Expectancy_Ratio_(期望)"] = amount
                        data[f"Expectancy_Ratio_(期望比率)"] = ratio

                    elif metric == 'Avg. daily profit':
                        metric_display = f"Avg._daily_profit_(日均利润)"  # 日均利润：每日平均盈利金额，日均受益
                    ...
                    elif metric == 'Days win draw lose' or metric == 'Days win/draw/lose': # Days win/draw/lose   
                        metric_display = f"Days_win_draw_lose_(胜平负天数)"  # 胜平负天数：盈利、持平、亏损的天数统计
                    elif metric == 'Min Max Avg. Duration Winners' or metric == 'Min/Max/Avg. Duration Winners': # Min/Max/Avg. Duration Winners
                        metric_display = f"Min_Max_Avg._Duration_Winners_(赢家交易的最小最大平均持仓时间)"  # 赢家交易的最小最大平均持仓时间：盈利交易的持仓时间统计
                    elif metric == 'Min Max Avg. Duration Losers' or metric == 'Min/Max/Avg. Duration Losers': # 
        ...
        # Parse STRATEGY SUMMARY
            ...
            if len(headers)!=8 or len(rows[0])!=8 :
                print(headers,rows)
                file_content = file_content # 'USDT_multi_tf_20200730_20250924_8h.txt'
            ...
            for row in rows:
                ...
                for header, value in zip(headers[1:], row[1:]):  # Skip Strategy column
                    ...
                    elif header == 'Avg Duration':
                        # value = parse_duration(value)  # 平均持仓时间：策略总结中的交易平均持续时间（小时）
                        header_display = 'Avg_Duration_(平均持仓时间)' # 交易时间相加取平均
                    elif header == 'Win  Draw  Loss  Win%':
                        header_display = f"Win_Draw_Loss_Win%_(策略胜平负及胜率)"  # 策略胜平负及胜率：特定策略的胜、平、负次数及胜率
                        # 胜了多少次？负了多次？平了多少次？胜的在总次数中的占比。
                        w, d, l, wp = value.split()
                        data[f"Win_Draw_Loss_Win%_胜"] = float(w)
                        data[f"Win_Draw_Loss_Win%_平"] = float(d)
                        data[f"Win_Draw_Loss_Win%_负"] = float(l)
                        data[f"Win_Draw_Loss_Win%_胜率"] = float(wp)
                    elif header == 'Drawdown':
                        header_display = 'Drawdown_(损失USDT_最大回撤)' # 最高点到最低点损失了多少钱
                    else:
                        header_display = f"{header_key}_({header})"

                    data[f"{header_display}"] = value

        i += 1
    # if not data:
    #     data = {'Strategy_(策略名称)': None, 'Trades_(策略交易次数)': None, 'Avg_Profit_%_(策略平均利润百分比)': None, 
    #             'Tot_Profit_USDT_(策略总利润USDT)': None, 'Tot_Profit_%_(策略总利润百分比)': None, 'Avg_Duration_(平均持仓时间)': None,
    #              'Win_Draw_Loss_Win%_(策略胜平负及胜率)': None, 'Drawdown_(最大回撤)': None}
...
def process_folder(input_folder, output_file):
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith('.txt'):
            with open(os.path.join(input_folder, filename), 'r', encoding='utf-8') as f:
                content = f.read()
                data = extract_data_from_file(content, filename)
                data['Filename_(文件名)'] = filename  # 文件名：输入的回测报告文件名
                # if '' not in data.columns.tolist():
                    # continue
                # if data['Drawdown_(损失USDT_最大回撤)'] is None and data['Win  Draw  Loss  Win%'] is None:
                #     continue

                # if data['Drawdown_start_(回撤开始时间)'] is None and data['Drawdown_end_(回撤结束时间)'] is None and data['Market change'] is None:
                #     continue
    ...
    # Summary_Total_profit_Pct_(总利润百分比)
    
    df['Summary_Total_profit_Pct_(总利润百分比)'] = (
    df['Summary_Total_profit_Pct_(总利润百分比)']
    .astype(str)              # 确保是字符串
    ...
    df = df[df['Summary_Total_profit_Pct_(总利润百分比)'] > 100]

    df = df[df['Summary_Sharpe_(夏普比率)'].astype(float) > 0]

    # Define columns to drop
    # columns_to_drop = [
    #     'Summary_Drawdown_start_(日期时间)',
    #     'Summary_Drawdown_end_(日期时间)',
    #     'Summary_Market_change_(市场变化)',
    #     'StrategySummary_Bandtastic_Trades_(策略交易次数)',
    #     'StrategySummary_Bandtastic_Avg_Profit_%_(策略平均利润百分比)',
    #     'StrategySummary_Bandtastic_Tot_Profit_USDT_(策略总利润USDT)',
    #     'StrategySummary_Bandtastic_Tot_Profit_%_(策略总利润百分比)',
    #     'StrategySummary_Bandtastic_Avg_Duration_(Avg Duration)',
    #     'StrategySummary_Bandtastic_Win_Draw_Loss_Win%_(策略胜平负及胜率)',
    #     'StrategySummary_Bandtastic_Drawdown_(Drawdown)',
    #     'Summary_Drawdown_start_(Drawdown start)',
    #     'Summary_Drawdown_end_(Drawdown end)',
    #     'StrategySummary_AwesomeStrategy_Trades_(策略交易次数)',
    #     'StrategySummary_AwesomeStrategy_Avg_Profit_%_(策略平均利润百分比)',
    #     'StrategySummary_AwesomeStrategy_Tot_Profit_USDT_(策略总利润USDT)',
    #     'StrategySummary_AwesomeStrategy_Tot_Profit_%_(策略总利润百分比)',
    #     'StrategySummary_AwesomeStrategy_Avg_Duration_(Avg Duration)',
    #     'StrategySummary_AwesomeStrategy_Win_Draw_Loss_Win%_(策略胜平负及胜率)',
    #     'StrategySummary_AwesomeStrategy_Drawdown_(Drawdown)'
    # ]
    
    # Drop the specified columns (ignore if not present)
